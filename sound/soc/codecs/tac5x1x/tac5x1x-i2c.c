...
#define LTCH_TO_MASK_STR_MAP(latch_reg, map) (\
	(struct interrupt_info){ \
		.reg = (latch_reg), \
...
static irqreturn_t irq_thread_func(s32 irq, void *dev_id)
{
...
#if LINUX_VERSION_CODE <= KERNEL_VERSION(6, 10, 6)
	s32 *read_regs;
#else
	u8 *read_regs;
#endif
...
#if LINUX_VERSION_CODE <= KERNEL_VERSION(6, 10, 6)
	read_regs = (s32 *)tac5x1x->irqinfo.latch_data;
	for (i = 0; i < latch_count; i++) {
		ret = regmap_read(tac5x1x->regmap, tac5x1x->irqinfo.latch_regs[i],
				  &read_regs[i]);
		if (ret) {
			dev_err(tac5x1x->dev,
				"interrupt: read failed for reg=0x%x",
				tac5x1x->irqinfo.latch_regs[i]);
			read_regs[i] = 0;
		}
	}
#else
	read_regs = (u8 *)tac5x1x->irqinfo.latch_data;
	ret = regmap_multi_reg_read(tac5x1x->regmap, tac5x1x->irqinfo.latch_regs,
				    read_regs, latch_count);
#endif
	if (ret) {
		dev_err(tac5x1x->dev,
	...
	for (i = 0; i < latch_count; i++) {
		dev_dbg(tac5x1x->dev, "reg=0x%0x, val=0x%02x",
			tac5x1x->irqinfo.latch_regs[i], read_regs[i]);
		latch_set |= read_regs[i] & 0xff;
	}
	...
	for (i = 0; i < latch_count; i++) {
		if (!read_regs[i])
			continue;
		process_one_interrupt(tac5x1x, i, read_regs[i]);
		read_regs[i] = 0;
	}
...
static s32 tac5x1x_register_interrupt(struct tac5x1x_priv *tac5x1x)
{
	...
	latch_regs = devm_kzalloc(tac5x1x->dev, latch_count * sizeof(u32),
				  GFP_KERNEL);
	latch_data = devm_kzalloc(tac5x1x->dev, latch_count * sizeof(s32),
				  GFP_KERNEL);
	...
	ret = devm_request_threaded_irq(tac5x1x->dev, tac5x1x->irqinfo.irq, NULL,
					irq_thread_func, IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
					"TAC-IRQ", tac5x1x);
	if (ret)
...
static SOC_VALUE_ENUM_SINGLE_DECL(tac5x1x_out2_config_enum, TAC5X1X_OUT2CFG0, 2, 0x7,
				  tac5x1x_output2_config_text, tac5x1x_output2_config_values);
...
static const struct snd_kcontrol_new tad5x1x_controls[] = {
	...
	SOC_SINGLE("ASI_RX_CH3_EN Playback Switch", TAC5X1X_PASIRXCH3, 5, 1, 0),
	SOC_SINGLE("ASI_RX_CH4_EN Playback Switch", TAC5X1X_PASIRXCH4, 5, 1, 0),
...
static s32 tac5x1x_soc_resume(struct snd_soc_component *component)
{
	...
	ret = regulator_bulk_enable(tac5x1x_num_regulators, tac5x1x_regulators);
	if (ret) {
			    ...
			    struct device_node *np)
{
	...
	case TAA5212:
	case TAC5212:
		if (fwnode_property_read_u32(tac5x1x->dev->fwnode, "ti,adc2-impedance",
					     &tac5x1x->adc_impedance[1]))
			dev_warn(tac5x1x->dev, "Fail to get ti,adc2-impedance value\n");
		fallthrough;
	case TAC5211:
	case TAC5111:
		if (fwnode_property_read_u32(tac5x1x->dev->fwnode, "ti,adc1-impedance",
					     &tac5x1x->adc_impedance[0]))
			dev_warn(tac5x1x->dev, "Fail to get ti,adc1-impedance value\n");
				    ...
				    struct tac5x1x_priv *tac5x1x)
{
	...
	ret = devm_regulator_bulk_get(dev, tac5x1x_num_regulators, tac5x1x_regulators);
	if (ret) {
			 ...
			 enum tac5x1x_type type)
{
	...
	if (tac5x1x->out2x_vcom_cfg != -1) {
		snd_soc_component_update_bits(tac5x1x->component,
					      TAC5X1X_OUT2CFG0, TAC5X1X_OUT2CFG0_VCOM_MASK,
					      tac5x1x->out2x_vcom_cfg);
	}
